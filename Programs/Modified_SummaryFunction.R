summary.mod <- function(object,...)
{
  ...
  if(x$conv==4|x$conv==12) {
    ...
    if(NPROB>0)
    {
      ...
      cat("\n")
      tmp_class.membership=as.numeric(tmp)%>%matrix(ncol=4)%>%data.frame()
      dimnames(tmp_class.membership)=dimnames(tmp)
    }
    ...
    tmp_link=as.numeric(tmp)%>%matrix(ncol=4)%>%data.frame()
    dimnames(tmp_link)=dimnames(tmp)
    
    dimnames(tmp_link)[[2]]=dimnames(tmp_class.membership)[[2]]=dimnames(tTable)[[2]]
    return(list("Longitudinal model"=invisible(tTable),"Class membership model"=tmp_class.membership,"Link function"=tmp_link))
  }
}
